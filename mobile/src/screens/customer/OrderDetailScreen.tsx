...
const LOCATION_POLL_INTERVAL_MS = 15 * 1000; // poll every 15 seconds
...
export default function OrderDetailScreen({ route, navigation }: { route: any; navigation: any }) {
  ...
  const fetchDriverLocation = useCallback(async () => {
    try {
      ...
      // location not yet available – ignore
    }
  }, [orderId]);

  // Poll for driver location when order is in transit
  useEffect(() => {
...
const styles = StyleSheet.create({
  ...
  locationUpdated: { fontSize: 11, color: '#888', marginTop: 6, textAlign: 'right' },
  mapPlaceholder: {
    height: 100,
    borderRadius: 12,
  ...
  mapPlaceholderText: { fontSize: 13, color: '#888', marginTop: 8 },
  ...
  locationBtnText: { color: '#fff', fontWeight: '600', fontSize: 13 },
  chatBtn: {
    flex: 1,
